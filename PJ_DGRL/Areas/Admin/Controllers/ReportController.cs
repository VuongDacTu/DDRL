...
namespace PJ_DGRL.Areas.Admin.Controllers
{
    public class ReportController : BaseController
    {
        ...
        public ReportController(DbDgrlContext context, IWebHostEnvironment env, IConverter converter)
        {
            _context = context;
            _env = env;
        ...
        public IActionResult Index(int? departmentId, int? semesterId, int? classId)
        {
            ...
            var @class = _context.Classes.Include(x => x.Department).Include(x => x.Students).ThenInclude(x => x.SumaryOfPoints).Where(x => x.IsDelete == false && x.DepartmentId == departmentId && x.SumaryOfPoints.Any(sp => sp.ClassId == x.Id && sp.SemesterId == semesterId)).ToList();
            if(classId !=null)
            {
            ...
            ViewBag.Lecturer = _context.Lecturers.OrderBy(x => x.IsLeader).Include(x => x.Position).Where(x => x.DepartmentId == departmentId && x.IsDelete == false).ToList();
        ...
        public IActionResult ConvertToPDF(int? departmentId, int? semesterId, int? classId)
        {
            ...
            var lecturer = _context.Lecturers.OrderBy(x => x.IsLeader).Include(x => x.Position).Where(x => x.DepartmentId == departmentId && x.IsDelete == false).ToList();
            ...
            else
            {
                 finalHtml= template.Replace("{{department}}", department)
                                       ...
                                       .Replace("{{items3}}", "")
                                       .Replace("{{class}}", "<td><p style="+"text-align: center;"+"><b>Đại diện ban cán sự lớp</b></p></td>");
            }
        ...
        public IActionResult ConvertAllToPDF(int? departmentId, int? semesterId)
        {
            ...
            string item2Template = System.IO.File.ReadAllText(_env.WebRootPath + @"\ReportItem2Template.html");
            string item3Template = System.IO.File.ReadAllText(_env.WebRootPath + @"\ReportItem3Template.html");
            string department = _context.Departments.Include(x => x.Classes).FirstOrDefault(x => x.Id == departmentId ).Name;
            ...
            foreach (var c in @class)
            {
                ...
                int stt = 1;
                    ...
                    var svdg = _context.Students.Include(x => x.SumaryOfPoints).Where(x => x.SumaryOfPoints.Any(x => x.SemesterId == semesterId) && x.ClassId == c.Id).ToList();

                    var svkdg = _context.Students.Include(x => x.SumaryOfPoints).Where(x => !x.SumaryOfPoints.Any(x => x.SemesterId == semesterId) && x.ClassId == c.Id).ToList();
                    var tsv = _context.Students.Where(x => x.ClassId == c.Id).ToList();
                    string dssvkdg = "";
                    foreach (var i in svkdg)
                    {
                        if (i.IsActive == _isActive.HoatDong())
                ...
                int stt2 = 1;
                    ...
                    var students = _context.Students.Include(x => x.SumaryOfPoints).Where(x => x.ClassId == c.Id).ToList();
                    var sum = students.Where(x => x.SumaryOfPoints.Any(x => x.SemesterId == semesterId)).ToList();

                    model2.Add(new ReportPDF2()
                    {
                        stt2 = stt2++,
                        class2 = c.Name + " (" + students.Count() + ")",
                        xs = sum.Where(x => x.SumaryOfPoints.Any(x => x.LastPoint >= 90)).Count(),
                        t = sum.Where(x => x.SumaryOfPoints.Any(x => x.LastPoint >= 80 && x.LastPoint < 90)).Count(),
                        k = sum.Where(x => x.SumaryOfPoints.Any(x => x.LastPoint >= 70 && x.LastPoint < 80)).Count(),
                        tbk = sum.Where(x => x.SumaryOfPoints.Any(x => x.LastPoint >= 60 && x.LastPoint < 70)).Count(),
                ...
                finalHtml = template.Replace("{{department}}", department)
                                        ...
                                        .Replace("{{items3}}", "")
                                        .Replace("{{class}}", "<td><p style=" + "text-align: center;" + "><b>Đại diện ban cán sự lớp</b></p></td>");
               listString.Add(finalHtml);
            }

            foreach (var html in listString)
            {
            ...
            using (var memoryStream = new MemoryStream())
            {
                using (var archive = new ZipArchive(memoryStream, ZipArchiveMode.Create, true))
                {
                    for (int i = 0; i < pdfBytesList.Count; i++)
                    {
                        ...
                        var zipEntry = archive.CreateEntry($"{@class[i].Name}.pdf", CompressionLevel.Fastest);
                        using (var zipStream = zipEntry.Open())
                        {
                            zipStream.Write(pdfBytesList[i], 0, pdfBytesList[i].Length);
                        }
                    }
                }
