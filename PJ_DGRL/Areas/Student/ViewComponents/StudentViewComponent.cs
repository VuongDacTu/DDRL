...
namespace PJ_DGRL.Areas.Student.ViewComponents
{
    public class StudentViewComponent : ViewComponent
    {
        ...
        public async Task<IViewComponentResult> InvokeAsync()
        {
            ...
            var students = _context.Students.Include(x => x.SumaryOfPoints.Where(x => x.SemesterId == semesterId)).OrderByDescending(x => x.SumaryOfPoints.FirstOrDefault().SelfPoint).Where(u => u.ClassId == LTLogin.ClassId).ToList();
